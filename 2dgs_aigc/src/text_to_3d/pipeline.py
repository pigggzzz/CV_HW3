...
def _mapping(cfg: Mapping[str, Any]) -> dict[str, Any]:
    ...
    ts = cfg.get("threestudio", {}) or {}
    seed = cfg.get("seed", 0)
    wname = (cfg.get("wandb", {}) or {}).get("name", None) or "objectB"

    prompt = ts.get("prompt", None)
...
def run_text_to_3d(config_path: str | Path) -> None:
    ...
    ts = cfg.get("threestudio", {}) or {}
    project_root = ts.get("project_root", None)
    cwd = abs_path(project_root) if project_root else None

    t0 = time.time()
    wandb_log(run, {"stage": "threestudio_train"})
    for i, raw in enumerate(ts.get("train_commands", []) or []):
        cmd = render_template(str(raw), mapping)
        run_cmd(cmd, cfg=cfg, cwd=cwd, log_dir=log_dir, name=f"threestudio_train_{i:02d}", check=True)

    wandb_log(run, {"stage": "threestudio_export"})
    for i, raw in enumerate(ts.get("export_mesh_commands", []) or []):
        cmd = render_template(str(raw), mapping)
        run_cmd(cmd, cfg=cfg, cwd=cwd, log_dir=log_dir, name=f"threestudio_export_{i:02d}", check=True)
